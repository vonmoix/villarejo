...
function EduCard({ edu, index }: { edu: (typeof degrees)[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="bg-surface2 border border-[var(--border-subtle)] rounded-lg p-8"
    >
      <p className="font-mono text-[0.6875rem] text-subtle tracking-[0.04em] mb-4">
        {edu.period}
      </p>
      <p className="font-display font-semibold text-[1.125rem] tracking-[-0.015em] text-ink leading-snug mb-2">
        {edu.degree}
      </p>
      ...
      <p className="text-[0.8125rem] text-subtle mt-2">{edu.location}</p>
      {edu.distinction && (
