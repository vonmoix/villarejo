...
function SkillGroup({ group, index }: { group: (typeof groups)[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      transition={shouldReduce ? { duration: 0 } : { duration: 0.55, delay: cardDelay, ease: expoOut }}
      whileHover={shouldReduce ? {} : { y: -3, transition: { duration: 0.22, ease: expoOut } }}
      className="rounded border border-transparent hover:border-[var(--border-subtle)] hover:bg-[var(--surface)] p-4 -m-4 transition-colors duration-300"
    >
        ...
        {group.tags.map((tag, tagIndex) => (
          <motion.span
            ...
            transition={shouldReduce ? { duration: 0 } : {
              duration: 0.28,
              delay: cardDelay + 0.2 + tagIndex * 0.04,
              ease: expoOut,
            }}
